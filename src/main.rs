...
use wordle_solver::{Dictionary, Guesser};
...
fn play<G: Guesser + Default>(Args { max_games, max_attempts, .. }: Args) -> (f64, usize) {
	...
	for answer in GAMES.lines().take(max_games) {
		let answer_bytes = answer
			.as_bytes()
			.try_into()
			.expect("Every word must be 5 ASCII characters.");

		let start = Instant::now();

		if let Some(n_attempts) = dictionary.play(G::default(), answer_bytes, max_attempts) {
...
fn setup_tracing(Args { log_level, show_total_elapsed, .. }: Args) {
	let subscriber = tracing_subscriber::fmt()
		...
		.with_file(false)
		.with_target(false)
		.with_max_level(log_level.unwrap_or(Level::INFO));
